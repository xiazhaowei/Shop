...
namespace Shop.Apis.Controllers
{
    ...
    public class WalletController : BaseApiController
    {
        ...
        [Authorize]
        [Route("WalletPay")]
        public async Task<BaseApiResponse> WalletPay([FromBody]WalletPayRequest request)
        {
            request.CheckNotNull(nameof(request));
            var currentAccount = _contextService.GetCurrentAccount(HttpContext);
            var walletinfo = _walletQueryService.Info(currentAccount.WalletId.ToGuid());
            ...
            {
                return new BaseApiResponse { Code = 400, Message = "付款金额为零" };
            }
            if (walletinfo.IsFreeze == Freeze.Freeze)
            {
                return new BaseApiResponse { Code = 400, Message = "钱包冻结，请用其他方式支付" };
            }
            if (walletinfo.Cash < request.CashPayAmount || walletinfo.ShopCash < request.ShopCashPayAmount)
            {
                return new BaseApiResponse { Code = 400, Message = "错误，余额不足" };
            }
        ...
        public async Task<BaseApiResponse> AcceptTransfer([FromBody]AcceptTransferRequest request)
        {
            request.CheckNotNull(nameof(request));
        ...
        [Authorize]
        [Route("Recharge")]
        public async Task<BaseApiResponse> Recharge([FromBody]RechargeRequest request)
        {
            request.CheckNotNull(nameof(request));
        ...
        [Authorize]
        [Route("AddBankCard")]
        public async Task<BaseApiResponse> AddBankCard([FromBody]AddBankCardRequest request)
        {
            request.CheckNotNull(nameof(request));
        ...
        [Authorize]
        [Route("ApplyWithdraw")]
        public async Task<BaseApiResponse> ApplyWithdraw([FromBody]ApplyWithdrawRequest request)
        {
            request.CheckNotNull(nameof(request));
        ...
        [Authorize]
        [Route("ApplyRecharge")]
        public async Task<BaseApiResponse> ApplyRecharge([FromBody]ApplyRechargeRequest request)
        {
            request.CheckNotNull(nameof(request));
