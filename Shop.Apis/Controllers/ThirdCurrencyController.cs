...
namespace Shop.Apis.Controllers
{
    ...
    public class ThirdCurrencyController:BaseApiController
    {
        ...
        public async Task<BaseApiResponse> ImportCurrency([FromBody]ImportCurrencyRequest request)
        {
        ...
        [Authorize]
        [Route("ImportLogs")]
        public BaseApiResponse ImportLogs([FromBody]ImportLogsRequest request)
        {
            request.CheckNotNull(nameof(request));
            var currentAccount = _contextService.GetCurrentAccount(HttpContext);
            var userInfo = _userQueryService.FindUser(currentAccount.UserId.ToGuid());
